    ...
    <v-card class="px-4 py-2 border" elevation="0">
        <v-row class="d-flex justify-end align-center my-2">
            ...
            <v-col cols="auto" class="d-flex justify-end">
                <cs_BtnAdicionar @click="CreateOrUpdateConvenio" />
            </v-col>
        </v-row>
        ...
        <v-card class="border mb-10" elevation="0">
            <v-row class="pa-0">
                <v-col cols="12">
                    <p class="text-h4 px-6 py-3 ma-0 rounded-t bg-primary">Convênio</p>
                </v-col>
            </v-row>
    ...
    <v-dialog v-model="confirmDialog" max-width="400">
        <v-card>
            ...
            <v-card-actions class="d-flex justify-space-around">
                ...
                <cs_BtnExcluir @click="deleteConvenioConfirmed" />
            </v-card-actions>
        </v-card>
    </v-dialog>
...
// Import de types
import type { ContaById, NavConvenioList } from '../../../types/crm/contas/bb012_contabyid';
...
import { AvalistaCreate } from '@/types/crm/contas/tabelasAuxiliares/bb01207_avalistas';
...
const var_TipRegistro = ref<number>(2);
...
const fetchData = async (id: string) => {
    ...
    try {
        ...
        console.log(res);
        items.value = res.Data.NavMembrosCompleto.NavConvenioList.map((item: NavConvenioList) => ({
            ID: item.Id,
            BB012_ID: item.Bb012Id,
            Convenio: item.NavBb012Membro.Bb012NomeCliente
        }));

        var_bb012_Id.value = res.Data.Id;
    } catch (error) {
        showSnackbar('Erro ao buscar convênio.', 'error');
    } finally {
        loading.value = false;
    }
};

const CreateOrUpdateConvenio = async () => {
    if (var_SelectedConta.value != '') {
        try {
            ...
            if (response.data.Out_IsSuccess) {
                showSnackbar('Convênio salva com sucesso', 'success');
                fetchData(props.id);
                var_SelectedConta.value = '';
            } else {
                showSnackbar(response.data.Out_Message || 'Falha ao salvar o convênio. Verifique os dados.', 'error');
            }
        } catch (error) {
            showSnackbar('Erro inesperado ao salvar o convênio', 'error');
        }
    } else {
...
const deleteConvenioConfirmed = async () => {
    ...
    try {
        ...
        showSnackbar('Convênio excluída com sucesso', 'success');
        fetchData(props.id);
        confirmDialog.value = false;
    } catch (error) {
