    ...
    <v-dialog v-model="dialog" max-width="500">
        <v-card>
            <v-card-title class="pa-4 bg-lightprimary">
                <span class="text-h5">{{ itemToEdit ? 'Editar' : 'Adicionar' }} Convênio</span>
            </v-card-title>
            <v-card-text>
                <v-form ref="formRef">
                    <InputTexto
...
// Import de types
import type { ContaById, Convenios } from '../../../types/crm/bb012_GetContaById';
...
const openEditDialog = async (item: Item) => {
    ...
    try {
        ...
        showSnackbar('Erro ao buscar dados do convênio', 'error');
    }
};
