<template>
    <v-container fluid class="border-sm rounded bg- ma-0">
        <div>
            <v-row class="pa-4 pt-7">
                <p class="text-h2 mb-5">Lista de Convênio Master</p>
                <v-divider></v-divider>
            </v-row>
        </div>
    ...
    <v-dialog v-model="dialog" max-width="400">
        <v-card>
            <v-card-title class="pa-4 bg-lightprimary">
                <span class="text-h5">{{ itemToEdit ? 'Editar' : 'Adicionar' }} Convênio Master</span>
            </v-card-title>
            <v-card-text>
                <v-form ref="formRef">
                    <div class="d-flex">
            ...
            <v-card-actions class="d-flex justify-space-around">
                ...
                <cs_BtnSalvar @click="CreateOrUpdateConvenioMaster" />
            </v-card-actions>
        </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="confirmDialog" max-width="400">
        <v-card>
            ...
            <v-card-text>Tem certeza de que deseja excluir esta convênio master?</v-card-text>
            <v-card-actions class="d-flex justify-space-around">
                <v-spacer></v-spacer>
                <cs_BtnCancelar @click="cancelDelete" />
                <cs_BtnExcluir @click="deleteConvenioMasterConfirmed" />
            </v-card-actions>
        </v-card>
    </v-dialog>
...
const var_bb067_Codigo = ref<string>('');
const var_bb067_Descricao = ref<string>('');
...
const openDialog = () => {
    ...
    var_ID.value = 0;
    var_bb067_Codigo.value = '';
    var_bb067_Descricao.value = '';
};
...
const openEditDialog = async (item: Item) => {
    ...
    try {
        const res: ConvenioMasterById = await GetConvenioMasterById(tenant, item.ID);
        const data = res.Data;
...
const deleteConvenioMasterConfirmed = async () => {
    if (!itemToDelete.value) return;
    try {
        await DeleteConvenioMaster(tenant, itemToDelete.value.ID);
        showSnackbar('Convênio master excluída com sucesso', 'success');
        fetchData();
        confirmDialog.value = false;
    } catch (error) {
        showSnackbar('Erro ao excluir o convênio master', 'error');
    }
};
